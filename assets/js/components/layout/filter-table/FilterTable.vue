            ...
            <div style="position: relative">
                <v-data-table
                    ...
                    <template v-for="headerItem in unscopedHeaderSlots" #[`header.${headerItem.value}`]="{ header }">
                        <v-tooltip v-if="header && header.tooltip" bottom>
                            <template #activator="{on}">
                                <span v-on="on" v-html="header.text"/>
                            </template>
                            <span v-html="header.tooltip"/>
                        </v-tooltip>
                        <span v-else v-html="header.text"/>
                    </template>
                    <template v-for="field in tableSlots" #[field]='allProps'>
                        <slot :name="field" v-bind="allProps"/>
                    </template>
                    <template v-for="unscopedDateField in unscopedDateSlots" v-if="unscopedDateSlots.length > 0" #[`item.${unscopedDateField}`]='allProps'>
                        <span class="no-whitespace">{{ allProps.item[unscopedDateField] | formatDate('dd MMM \'yy') }}</span>
                        <br/>
                        <span class="no-whitespace">{{ allProps.item[unscopedDateField] | formatDate('HH:mm') }}</span>
                    </template>
